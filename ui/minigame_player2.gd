...
func _input(event):
	if event.is_action_pressed("move_right_p2"):
		get_viewport().set_input_as_handled()
		_navigate(1, 0)
	elif event.is_action_pressed("move_left_p2"):
		get_viewport().set_input_as_handled()
		_navigate(-1, 0)
	elif event.is_action_pressed("move_backward_p2"):
		get_viewport().set_input_as_handled()
		_navigate(0, 1)
	elif event.is_action_pressed("move_forward_p2"):
		get_viewport().set_input_as_handled()
		_navigate(0, -1)
	elif event.is_action_pressed("ui_accept_p2") and not event.is_echo():
		get_viewport().set_input_as_handled()
		_activate_selected()
...
func _navigate(dx: int, dy: int):
	var columns = 3
	var rows = ceili(float(buttons.size()) / float(columns))
	var row = floori(float(selected_index) / float(columns))
	var col = selected_index % columns
	for step in range(1, max(columns, rows)):
		var target_row = row + dy * step
		var target_col = col + dx * step
		if target_row < 0 or target_row >= rows or target_col < 0 or target_col >= columns:
			continue
		var test_index = target_row * columns + target_col
		if test_index >= buttons.size():
			continue
		if not buttons[test_index].disabled:
