...
@onready var bg_music_slider: HSlider = $Part2/MarginContainer/VBoxContainer/MarginContainer/VBoxContainer/bg_music_slider
@onready var sfx_volume_slider: HSlider = $Part2/MarginContainer/VBoxContainer/MarginContainer2/VBoxContainer/sfx_volume_slider
...
@onready var margin_container_3: MarginContainer = $Part2/MarginContainer/VBoxContainer/MarginContainer3
@onready var points_required_slider: HSlider = $Part2/MarginContainer/VBoxContainer/MarginContainer3/VBoxContainer/PointsRequiredSlider

@onready var bg_value: Label = $Part2/MarginContainer/VBoxContainer/MarginContainer/VBoxContainer/HBoxContainer/BGValue
@onready var sfx_value: Label = $Part2/MarginContainer/VBoxContainer/MarginContainer2/VBoxContainer/HBoxContainer/SFXValue
@onready var points_value: Label = $Part2/MarginContainer/VBoxContainer/MarginContainer3/VBoxContainer/HBoxContainer/PointsValue
...
func _ready() -> void:
	...
	# --- Sliders ---
	bg_music_slider.value = SFX.music_volume * 100
	sfx_volume_slider.value = SFX.sfx_volume * 100
	points_required_slider.value = GameData.points_required
	
	# Initialize value labels
	bg_value.text = str(int(bg_music_slider.value))
	sfx_value.text = str(int(sfx_volume_slider.value))
	points_value.text = str(int(points_required_slider.value))

	bg_music_slider.value_changed.connect(_on_music_slider_changed)
	sfx_volume_slider.value_changed.connect(_on_sfx_slider_changed)
	points_required_slider.value_changed.connect(_on_points_required_slider_changed)
...
func _on_points_required_slider_changed(value: float):
	GameData.points_required = int(value)
	points_value.text = str(int(value))
	SFX.play_move()
	print("Points required to win set to: ", GameData.points_required)
...
func _on_start_game_as_is_pressed():
	...
	options_game.visible = false
	tutorial_for_import.visible = false
	_start_minigame("res://assets/minigames/Minigame_2.tscn", "minigame_2")
...
func _on_file_selected(path: String):
	...
	# ✅ FIX 1.5: Unwrap word-wrapped text first
	# This handles text where questions/answers are split across multiple lines
	# Replace single newlines (word wraps) with spaces, but keep double newlines (question separators)
	var lines = content.split("\n")
	var unwrapped_lines: Array[String] = []
	var current_line = ""
	
	for i in range(lines.size()):
		var line = lines[i].strip_edges()
		
		# Check if this line starts a new question/answer marker
		var is_marker = (line.begins_with("Q:") or line.to_upper().begins_with("Q:") or
						 line.begins_with("A:") or line.to_upper().begins_with("A:") or
						 line.begins_with("B:") or line.to_upper().begins_with("B:") or
						 line.begins_with("C:") or line.to_upper().begins_with("C:") or
						 line.begins_with("D:") or line.to_upper().begins_with("D:") or
						 line.to_upper().begins_with("ANSWER:"))
		...
		if is_marker:
			...
			# Empty line - preserve as separator
			if current_line != "":
				unwrapped_lines.append(current_line)
				current_line = ""
			unwrapped_lines.append("")
		else:
			# Continuation of previous line (word wrap)
			if current_line == "":
				current_line = line
			else:
				current_line += " " + line
	...
	# ✅ FIX 1.6: Now handle single-line format (all on one line with spaces)
	if content.contains(" Q:") and not content.contains("\nQ:"):
	...
	FadeManager.fade_to_scene("res://assets/minigames/Minigame_2.tscn")
